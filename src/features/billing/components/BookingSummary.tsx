...
export default function BookingSummary({ customer, workflowType, items, selectedLens, availability, availabilityOverrideDecision, availabilityOverrideReason, onAvailabilityOverrideDecision, onAvailabilityOverrideReason, received }: BookingSummaryProps) {
  ...
  const needsAttention = workflowType === "PRESCRIPTION" && (!frame || !selectedLens || !availability || (availability.decision === "REVIEW_REQUIRED" && !availabilityOverrideDecision));
  ...
  return <Card className="billing-booking-summary">
    ...
    <div className="billing-booking-summary__grid">
      ...
      <SummaryItem label="Expected delivery" value={availability?.expectedDeliveryDate ? new Intl.DateTimeFormat("en-IN", { day: "numeric", month: "short", year: "numeric" }).format(new Date(`${availability.expectedDeliveryDate}T00:00:00`)) : "Not scheduled"} detail={availability ? availability.deliveryState.toLowerCase() : "Profile assessment pending"} />
